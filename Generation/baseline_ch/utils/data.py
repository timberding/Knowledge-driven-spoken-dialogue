...
def write_generation_preds(dataset_walker, output_file, dialog_ids, responses):
    new_labels = []
    rel_to_test = {}
    for idx, (history, response, knowledge, dialog_id) in enumerate(dataset_walker):
        rel_to_test[dialog_id] = {
            "attrs": knowledge,
            "message": responses[idx].replace(" ", "")
        }
        new_labels.append(
            {
                ...
                'generated_response': responses[idx].replace(" ", ""),
            }
        )
    ...
    with open(output_file, "w") as jsonfile:
        logger.info("Writing predictions to {}".format(output_file))
        json.dump(new_labels, jsonfile, indent=2, ensure_ascii=False)

    with open("to_test_"+output_file, "w") as jsonfile:
        json.dump(rel_to_test, jsonfile, indent=2, ensure_ascii=False)
