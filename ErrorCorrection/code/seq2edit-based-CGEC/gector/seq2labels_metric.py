...
class Seq2LabelsMetric(Metric):
    ...
    def __call__(self, logits_labels, labels, logits_d, d_tags, mask=None,
                 crf_result=None):
        logits_labels, labels, logits_d, d_tags, mask = self.detach_tensors(
            logits_labels, labels, logits_d, d_tags,
            mask)
        ...
        correct_labels_ueq_keep = (argmax_labels.eq(
            labels.unsqueeze(-1)) & labels_ueq_keep).float()
    ...
    def Labels_Accuracy_Except_Keep(self):
        return self.labels_without_keep_correct / (
                    self.labels_without_keep + self.eps)
    ...
    def get_metric(self, reset: bool = False, model_object=None):
        ret = {"labels_accuracy": self.Labels_Accuracy,
               "d_tags_accuracy": self.Tags_Accuracy,
               "labels_accuracy_except_keep": self.Labels_Accuracy_Except_Keep,
